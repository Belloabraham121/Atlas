...
import { bus, A2AMessage } from "../utils/bus.js";

export class HederaLLMAgent {
  private agentName = "llm@portfolio.guard";
  ...
  private async handleMessage(message: A2AMessage): Promise<void> {
    switch (message.type) {
      ...
      case "analysis_response":
        console.log(
          `🤖 LLM Agent processing analysis response from ${message.from}`
        );
  ...
  private async handleLLMQuery(message: A2AMessage): Promise<void> {
    try {
      ...
      if (addressAnalysis.isAddressAnalysis && addressAnalysis.address) {
        console.log(
          `🔍 Detected address analysis request for: ${addressAnalysis.address}`
        );
        const response = await this.handleAddressAnalysis(
      ...
      if (!this.llm) {
        ...
        try {
          ...
          const systemMessage =
            new SystemMessage(`You are PortfolioGuard, an AI assistant specialized in Hedera network portfolio management and analysis.
...
Keep responses concise but informative.`);
  ...
  private parseAddressAnalysisRequest(query: string): {
  ...
  private async handleAddressAnalysis(
    address: string,
    userId: string,
    ...
    try {
      ...
      const analysisData = await new Promise<any>((resolve, reject) => {
        ...
        const responseHandler = (responseMessage: A2AMessage) => {
          if (
            responseMessage.type === "analysis_response" &&
            ...
            if (responseMessage.payload.success) {
              ...
              reject(
                new Error(responseMessage.payload.error || "Analysis failed")
              );
            }
          }
        };
...
Try asking me to "analyze my portfolio" or "check my balances"!`;
  ...
  private async getPortfolioContext(userId: string): Promise<string> {
    try {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      if (process.env.HEDERA_OPERATOR_ID && process.env.HEDERA_OPERATOR_KEY) {
        ...
        console.log(
          "⚠️ Hedera operator credentials not found. Portfolio queries will be limited."
        );
      }
  ...
  getStatus(): { name: string; status: string; initialized: boolean } {
    return {
      name: this.agentName,
