...
export function useEvmToHederaAccount(): UseEvmToHederaAccountReturn {
   ...
   */
  const convertEvmToHedera = useCallback(
    async (evmAddress?: string): Promise<void> => {
      ...
      try {
        ...
        if (!isValidEvmAddress(cleanAddress)) {
          throw new Error(
            "Invalid EVM address format. Please provide a valid Ethereum address."
          );
        }
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Failed to convert EVM address to Hedera account ID";
 ...
 */
export async function convertEvmAddressToHederaAccountId(
  evmAddress: string
): Promise<string> {
  try {
    ...
    // Validate format
    const cleanHex = cleanAddress.startsWith("0x")
      ? cleanAddress.slice(2)
      : cleanAddress;
    if (!/^[0-9a-fA-F]{40}$/.test(cleanHex)) {
      throw new Error(
        "Invalid EVM address format. Must be a 40-character hex string."
      );
    }
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Failed to convert EVM address";
