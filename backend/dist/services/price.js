...
export async function fetchPriceUSD(tickerOrId) {
...
export async function fetchTokenMarketData(tickerOrId) {
    try {
        const id = TICKER_TO_COINGECKO[tickerOrId.toUpperCase()] || tickerOrId;
...
export async function fetchHistoricalPrices(tickerOrId, days = 30, interval = 'daily') {
    try {
        const id = TICKER_TO_COINGECKO[tickerOrId.toUpperCase()] || tickerOrId;
...
export async function fetchTokenAnalytics(tickerOrId, timeframe = '24h') {
    try {
...
function determineTrend(prices, priceChange24h) {
    if (prices.length < 2) {
    ...
    if (upCount > downCount && priceChange24h > 1)
        return 'bullish';
    if (downCount > upCount && priceChange24h < -1)
        return 'bearish';
    return 'neutral';
}
