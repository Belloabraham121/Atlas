...
export class GraphAgent {
    ...
    async handleMessage(message, bus) {
        try {
            switch (message.type) {
                case 'generate_graph':
            ...
            this.sendErrorResponse(message, bus, error);
        }
    }
    async generateGraph(message, bus) {
        try {
    ...
    async generateTokenChart(message, bus) {
        try {
    ...
    async generatePortfolioChart(message, bus) {
        try {
    ...
    async generateCorrelationChart(message, bus) {
        try {
    ...
    sendErrorResponse(message, bus, error) {
        bus.sendMessage({
            type: 'graph_error',
            from: this.agentName,
            to: message.from,
    ...
    createChartConfiguration(userId, timeframe) {
        return this.generateChartConfig(userId, timeframe);
    }
    createTokenChartConfig(tokenChartData) {
    ...
    createPortfolioValueChart(portfolioData) {
        ...
        const timestamps = portfolioData.tokens[0]?.data.map(point => new Date(point.timestamp).toLocaleDateString()) || [];
    ...
    createTokenPerformanceChart(portfolioData) {
        const tokenPerformance = portfolioData.tokens.map(token => {
            const data = token.data;
            if (data.length < 2)
                return { symbol: token.symbol, change: 0 };
        ...
        return {
            ...
            data: {
                ...
                datasets: [{
                        ...
                        backgroundColor: tokenPerformance.map(t => t.change >= 0 ? '#10b981' : '#ef4444')
                    }]
            },
            options: {
                responsive: true,
    ...
    createCorrelationChartConfig(correlationData) {
        const scatterData = [];
        ...
        for (let i = 0; i < correlationData.tokens.length; i++) {
            for (let j = i + 1; j < correlationData.tokens.length; j++) {
                ...
                const dataPoints = token1Data.map((point, index) => ({
                    x: point.price,
    ...
    generateChartConfig(userId, timeframe) {
